...
Written as: $X_n \overset{L^r}{\to} X$

Proof: Assume $X_n \overset{L^r}{\to} X$. We know then that $\lim_{n \to \infty} \EX(|X_n - X|^r) = 0$. 
...
We showed previously that $X_n \overset{P}{\to} 0$ but $X_n \cancel{\overset{a.s.}{\to}} 0$
What about $X_n \overset{L^r}{\to} 0$?
...
Thus the limit goes to 0, which then means that $X_n \overset{L^r}{\to} 0$. This also implies $X_n \overset{P}{\to} 0$\\~\\

If $s > r \ge 1$ then $X_n \overset{L^s}{\to} X \implies X_n \overset{L^r}{\to} X$
...
$X_n \overset{D}{\to} X$ is implied by all the other convergences (probability, almost surely, and $r^{th}$ means).
...
\end{math}
\obeylines\\
Note that this is the CDF for a point mass (step function) at 1, so $X_{(n)} \overset{D}{\to} 1$.


Theorem: For a sequence of random variables $X_{(n)} \overset{P}{\to} C$ iff $X_{(n)} \overset{D}{\to} C$
...
Thus $Y_n \overset{D}{\to} Y \sim Exp(1)$

\subsection{Central Limit Theorem}
Consider $X_n = \frac{1}{n} \sum_{i=1}^{n} X_i$ for a sequence $X_1, X_2, ...$.
We have shown: $\bar{X_n} \overset{P}{\to} \mu$, $\bar{X_n} \overset{a.s.}{\to} \mu$
...
Thus, $\frac{\sqrt{n}(\bar{X_n} - \mu)}{\sigma} \overset{D}{\to} N(0,1)$, or $Z_n = \frac{\sqrt{n}(\bar{X_n} - \mu)}{\sigma}$ and $Z \sim N(0,1), Z_n \overset{D}{\to} Z$
