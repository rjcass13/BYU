...
where $\mathbf{x}_i'$ is the matrix representing the explanatory variables, and $\beta$ is the matrix representing the coefficients for each factor.
...
This model assumes that each event is indepenent. It also assumes that each factor is monotonic (always increasing or decreasing) in relation to the likelihood of a client opening an account. If these assumptions are not met, the model will not accurately capture the effect of each factor and the significance of each variable will be wrong.