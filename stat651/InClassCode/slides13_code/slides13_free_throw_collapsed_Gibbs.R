...
upd_M <- function(M_old, y, n, mu, sh_M, sc_M, cand_sig) {
  
  logtarget <- function(lM) {
    M <- exp(lM)
    alpha <- M * mu
    beta <- M * (1.0 - mu)
    sum(lbeta(alpha + y, beta + n - y)) - # log marginal "likelihood"
      N * lbeta(alpha, beta) + # log marginal likelihood
...
upd_mu <- function(mu_old, y, n, M, sh1_mu, sh2_mu, w = 0.2) {
  ...
  logtarget <- function(mu) {
    if (mu < 0.0 || mu > 1.0) {
      ...
      alpha <- M * mu
      beta <- M * (1.0 - mu)
      out <- sum(lbeta(alpha + y, beta + n - y)) - # log marginal "likelihood"
        N * lbeta(alpha, beta) + # log marginal "likelihood"
...
for (i in 1:n_iter) {
  ...
  tmp <- upd_M(M_old = state$M, y = y, n = n, mu = state$mu, 
               sh_M = sh_M, sc_M = sc_M, cand_sig = 1.0)

  accpt_tot <- accpt_tot + tmp$accpt
  state$M <- tmp$M
  
  tmp <- upd_mu(mu_old = state$mu, y = y, n = n, M = state$M, 
...
## add thetas in post processing

sims_theta <- matrix(NA, nrow = n_iter, ncol = N)
for (i in 1:n_iter) {
  sims_theta[i,] <- rbeta(N, sims[i,"M"] * sims[i,"mu"], sims[i,"M"] * (1.0 - sims[i,"mu"]))  
}

plot(as.mcmc(sims_theta)) # will show 10 plots, a few at a time

hist(sims_theta[,4], freq = FALSE, xlim = c(0,1)) # posterior of thete_4 for fourth player who made 4/4 free throws
...
sims_ypred4 <- rbinom(n_iter, size = 5, prob = sims_theta[,4])
